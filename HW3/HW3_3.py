...
def online_sequence(covariance, mu , n, p_variance, x_matrix, y_value,mean):
    ...
    for i in range(100):
      ...
      mean_previous   = mean_data
      mean_data       = compute_next_mean(mean_previous,  value[0] , len(f_y))
      variance_data   = compute_next_variance(variance_data, mean_previous, mean_data, value[0], len(f_y))
      temp_a          = 1 / variance_data
      ...
      covariance = temp_a *  np.dot( np.transpose(x_matrix),x_matrix) + inverse(covariance)
      temp       = np.dot(temp_a *  np.transpose(x_matrix),y_value) + np.dot(inverse(covariance),mu)
      mu         = np.dot(inverse(covariance), temp)
      
      p_mean     = np.dot(temp_x_matrix,mu)[0]
      
      p_variance = 1/temp_a + np.dot(np.dot(temp_x_matrix,inverse(covariance)),np.transpose(temp_x_matrix))[0,0]
...
def f2(t1, mu, covariance, mode):
    ...
    for i in range(t1.shape[0]):
        ...
        variance = a + np.dot(np.dot(temp_x_matrix,inverse(covariance)),np.transpose(temp_x_matrix))[0,0]
...
def compute_next_mean(previous_mean, x , n):
    return previous_mean + (x - previous_mean) / n

def compute_next_variance(previous_variance, previous_mean, now_mean , x, n):
    return previous_variance + ((x - previous_mean) * (x - now_mean) - previous_variance) / n
...
if __name__ == '__main__':
    ...
    mean_data    = compute_next_mean(mean_data,  first_value[0] , len(f_y))
    variance_data   = compute_next_variance(variance_data, 0, mean_data, first_value[0], len(f_y))
