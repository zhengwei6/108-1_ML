...
class Gaussian_Process(object):
    ...
    def optimze_process(self):
        sol = minimize(self.objective,self.hypm, method='Nelder-Mead',options={'disp':True})
        self.hypm = sol.x
        print(sol.x)
        return
...
def rational_quadratic(xa,xb,x):
    sq_norm = x[0]**2*(1 + (1/x[1])*0.5*scipy.spatial.distance.cdist(xa, xb, 'sqeuclidean'))**(-x[1])
    return sq_norm
...
if __name__ == '__main__':
    ...
    ax3.fill_between(X2.flat, mu2-2*Lsigma2, mu2+2*Lsigma2, color='red', alpha=0.15, label='$postior$')
